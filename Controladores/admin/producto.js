...
/*
*   Función asíncrona para mostrar un gráfico de los 5 productos con más existencias.
*   Parámetros: ninguno.
*   Retorno: ninguno.
*/
const openTopProductsChart = async () => {
    // Petición para obtener los datos de los productos con más existencias.
    const DATA = await fetchData(PRODUCTO_API, 'readTopProductos', null);
    ...
    if (DATA.status) {
        ...
        grafiProducto('chart', productos, existencias, 'Top 5 productos con más existencias');
    } else {
        sweetAlert(4, DATA.error, true);
    }
}
